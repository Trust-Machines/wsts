...
As opposed to FROST, each party is given not only a party ID $i_p \in [1, N_p]$, but also a set of key IDs $i_k \in [1, N_k]$.  We denote the latter set as $K_p$.

Each party $i \in [1, N_p]$ begins DKG by generating a random scalar polynomial $f_i$ with scalar coefficients.  They then construct DKG public shares by multiplying the coefficients of $f_i$ by the generator, to obtain $P_i$.
...
Once a party receives its DKG public shares, it first must verify that each share is valid using a similar formula as before:
\begin{align}
  \forall j_p \in [1,N_p], i_k \in K_p; f_{j_p}(i_k) \cdot G = P_{j_p}(i_k)
\end{align}
...
\begin{align}
  s_{i_k} = \sum_{j=1; j \neq i}^{N} f_j(i) ;\; Y_{i_k} = s_{i_k} \cdot G
\end{align}
...
At this point DKG is complete, and each party $i$ knows all of its $(s_{i_k}, Y_{i_k})$.
