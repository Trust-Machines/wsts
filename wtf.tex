...
As opposed to FROST, each party is given not only a party ID $i_p \in [1, N_p]$, but also a set of key IDs $i_k \in [1, N_k]$.  We denote the latter set as $K_{i_p}$.
...
Next they compute the interpolation value $\lambda_{i_k}$:
\begin{align}
  \lambda_i = \prod_{j=1; j \neq i}^{N_k} \frac{j}{j - i}
\end{align}
