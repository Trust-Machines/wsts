...
The parties then evaluate their own $f$ at each $j \in [1,N]$, and send a private DKG share to each consisting of $(j, f_i(j))$.  Each receiving party checks each private share for consistency with the public polynomial of the sending party.  This is done by checking that for party $i$, $\forall j \in [1,N]$:
\begin{align}
  f_j(i) * G = P_j(i)
\end{align}
