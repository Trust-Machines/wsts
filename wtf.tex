...
In addition to WTF, here we also present FROST Interactive Robustness Extension (FIRE).  This is a straightforward extension of FROST into a meta protocol which provides robustness.  It works with both FROST and WTF, with minor differences in the protocol.
...
ROAST\cite{roast} is a wrapper around FROST and other threshold signature schemes which provides robustness and asynchronicity.  It operates in a very similar manner to FIRE, but the asynchonicity fixes a byzantine flaw in FIRE.

This flaw is that once a party has given a nonce, there is no way to force that party to provide a signature.  And given network constraints, we cannot know whether a party in $S_j$ is refusing to sign, or if the signature was lost in the tubes.  So we must set a timeout, which allows a set of byzantine actors to slow the protocol significantly.  For a given timeout $t$, $N$ keys, and threshold $T$, a set of byzantine actors can force the amount of time for round $i$ to $t \cdot (N - T)$.  This is of course suboptimal.

ROAST fixes this by allowing session $j+1$ to run in parallel with session $j$.  To do so, it is necessary to curate the set $A$ more actively, and also introduce a set $B$ of known bad parties.  Once a party becomes a member of set $B$, all messages from that party will be ignored, and if the governance model includes sanctions on bad actors, those in $B$ will be nominated for such.

So as with FIRE, signing round $i$ begins session $j = 0$ by asking all parties in $A$ for nonces.  Crucially, these members are removed from set $A$, which becomes the null set.  As parties return nonces, they are placed back into $A$.  When $|A| = T$, we create set $S_j$, which is the signing set for session $j$.  Again, parties who are selected for $S_j$ are removed from $A$.  But while session $j$ continues, anyone returning nonces will still be placed back in set $A$, though their nonces will be ignored.

Once a party $p$ returns a signature share, it must be checked for validity.  If it is valid, then $p$ is placed back into $A$, otherwise $p$ is placed into $B$.  Once $|A| = T$, then we begin session $j+1$, regardless of whether session $j$ has completed.  Crucially, when $N - |B| < T$, then signing round $i$ has failed, since it is impossible to create a valid group signature.  As before, if all parties in $S_j$ return a valid signature share, and the aggregate signature is also valid, then signing round $i$ has completed.

When session $j+1$ begins, all parties who are now in $A$ are asked for nonces, then removed from $A$.  As before, parties who return a nonce for session $j+1$ are placed back into $A$.  Parties who return a nonce for a session other than the current $j+1$ are placed into $B$, and ignored for the remainder of round $i$.  

The rest of the protocol is the same.  Without timeouts, there is no way for byzantine actors to slow the protocol.  We will converge to either a successful aggregate signature, or fail when $N - |B| < T$.

As with FIRE, we can use either FROST or WTF as the signature aggregation protocol, with the only difference being how we count the nonces when determining if we have a set of possible signers.
