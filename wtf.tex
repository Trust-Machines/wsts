...
So when round $i$ begins, we request nonces for session $j = 0$, where the set of active parties $A$ contains all $N$ parties who took part in DKG.  Once we get $k = T$ nonces for $i,j$, we request signature shares from those $k$ parties (which forms $S_j$).  If all sign correctly, and we obtain a valid signature, round $i$ is complete.

If some parties do not sign properly, they are removed from $A$, then we begin session $j+1$.  All members of $A$ who have by now returned nonces for session $j$ are then requested to give nonces for session $j+1$.  The same process happens as before; when we get $k = T$ nonces we form $S_{j+1}$ and again request signature shares.
...
ROAST\cite{roast} is a wrapper around FROST and other threshold signature schemes which provides robustness and asynchronicity.  It operates in a very similar manner to FIRE, but the asynchonicity fixes one glaring byzantine flaw in FIRE.
...
ROAST fixes this by allowing session $j+1$ to run in parallel with session $j$.  The parties which give valid signatures in session $j$ are added to the set of parties $B_j$ who gave a nonce for session $j$ but were not selected for $S_j$.  Once $|B_j| \geq T$, then we begin session $j+1$ immediately, while still awaiting signatures from the parties in $S_j$ who have not responded yet.

The rest of the protocol is the same.  We no longer worry about tracking $A$, since bad or slow signers will not be placed in $B_{j+1}$.  And without timeouts, there is no way for byzantine actors to slow the protocol.  We will converge to either a successful aggregate signature, or fail when for some session $j$, $|B_j| < T$.
