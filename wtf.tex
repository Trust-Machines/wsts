...
A Schnorr proof is a zero knowledge proof of ownership of a public key.  Let scalar $x$ be a private key, with point $X = x \cdot G$ the corresponding public key.  To prove knowledge of $x$ given $X$, Schnorr proofs use a $3$-move challenge-response protocol, i.e. a Sigma protocol.
...
To make this protocol non-interactive, use the Fiat-Shamir transform to construct $c$ by hashing the initial proof elements $(G, X, V)$:
\begin{align}
  c = H(G, X, V)
\end{align}
